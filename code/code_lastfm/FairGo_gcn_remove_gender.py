...
path_model_save='./pretrainmodel/model_gender.dat'
...
# copyfile('./FairGo_gcn_remove_gender.py', path_save_base+'/FairGo_gcn_remove_gender'+run_id+'.py')
...
ap=training_u_i_set[8403]
print(training_u_i_set[8403])
...
# pdb.set_trace()
shiyanmiaoshu='性别下FairtGo,lamd=20,RecFair，'
print(shiyanmiaoshu)
f = open('lightGCN_remove_gender_result1.txt', 'a')
...
class InforMax(nn.Module):
    ...
    def forward(self, adj_pos1,user_batch,rating_batch,item_batch,flag_t):      
        ...
        user_b = user_f1_tmp
        item_person_f = item_f1_tmp#gcn_items_embedding0#item_f1_tmp
        #user_b = F.embedding(user_batch,user_f1_tmp)
        ...
        g_loss_all = 10 * loss_p_square -  200*d_loss_all
        g_d_loss_all = - 1 * d_loss_all
        d_g_loss = [d_loss_all, g_loss_all, g_d_loss_all]
    ...
    def embed(self, adj_pos1): 
        ...
        return gcn_users_embedding0.detach(),fliter_u_emb1.detach(),gcn_items_embedding0.detach(),fliter_i_emb1.detach()
    def save(self, fn):
...
d_optimizer = torch.optim.Adam(list(model.model_d1.parameters())  ,lr=0.005)
...
for epoch in range(300):
    ...
    model.eval()
    user_emb_get,f1_u_emb,item_emb_get,f1_i_emb= model.embed(copy.deepcopy(pos_adj))
    ...
    user_e=user_e_f1#(user_e_f1+user_e_f2)/2.0
    item_e=item_e_f1#(item_e_f1+item_e_f2)/2.0
    # if epoch>0:
    ...
    #         str_print_evl+=str(round(i_f1,4))+' '
        ...
        # f1p_one,f1r_one,f1res_p,f1res_r= pc_age_train.clf_age_all_pre(run_id,epoch,user_e_f2,factor_num)
