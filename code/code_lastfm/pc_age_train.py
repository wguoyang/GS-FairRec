...
def clf_age_all_pre(model_id,epoch_run,users_embs,factor_num):
    ...
    # copyfile('./pc_age_train.py', path_save_base+'/pc_age_train.py')
    ...
    train_loader = DataLoader(train_dataset,
            batch_size=training_count, shuffle=True, num_workers=8,pin_memory=True)
    ...
    testing_loader = DataLoader(testing_dataset_loss,
            batch_size=testing_count, shuffle=False, num_workers=4,pin_memory=True)
    ...
    val_loader = DataLoader(val_dataset_loss,
            batch_size=val_count, shuffle=False, num_workers=4, pin_memory=True)

    ######################################################## TRAINING #####################################
    # print('--------training processing-------')
    count, best_hr = 0, 0 
    model = Classifier(embed_dim=factor_num, out_dim=3)
    model=model.to('cuda')  
    ...
    for epoch in range(230):
        ...
        for user_features, user_labels in testing_loader:
            ...
            str_f1='age ,f1_macro:'+str(round(f1_macro,4))+'  f1_micro:'+str(round(f1_micro,4))#+' auc:'+str(round(auc_test,4))
            if flag_stop>=2:
