...
user_num=5000#user_size
item_num=100#item_size
class RW():
    def __init__(self,X,Y,R,alpha):
        XX, YY = ['user_' + str(x) for x in X], ['item_' + str(y) for y in Y]
        # print(X)
        # print(Y)
        self.uimatrix=csr_matrix((R,(X,Y)),shape=(user_num+1,item_num+1))
        self.usersum=np.array(self.uimatrix.sum(axis=1)).squeeze()
        self.itemsum=np.array(self.uimatrix.sum(axis=0)).squeeze()
        self.alpha=alpha
        #print(self.uimatrix[1,661])3
        self.QG = self.get_graph(XX, YY)
        #item_51，item_91，item_109,item_115，142，143没有邻居节点
        ...
        if isuser:
            ...
            for id in range(user_num):
                ...
                for it in one_order.keys():
                    ...
                    iid=int(item[5:])
                    item=item[:5]+str(iid-1)
                    user_adj_dict[user][item]=score_dict[it]
            np.save('../data/sushi3/RW_adj_score_user.npy',user_adj_dict)
        else:
            item_adj_dict = {}
            for id in range(item_num):
                ...
                try:
                    ...
                    continue
                for it in one_order.keys():
                    user = copy.deepcopy(it)
                    uid = int(user[5:])
                    user = user[:5] + str(uid - 1)
                    item_adj_dict[item][user] = score_dict[it]
            np.save('../data/sushi3/RW_adj_score_item.npy', item_adj_dict)
    ...
    #     return user_agg
# r_zero=np.zeros(shape=(603082,1),dtype=int)
# r_zero[3,0]=1
# #r_zero[1,1]=1
# mat_all=np.load('../data/lastfm/RW_adj_score/mat_all.npy', allow_pickle=True).item()
# print(mat_all.shape)
# t0=time.time()
# #res=cgs(mat_all,r_zero)
#
# res = gmres(mat_all, r_zero, tol=1e-5)[0]
# t1=time.time()
# print(t1-t0)

train_rating,count=np.load('../data/sushi3/train_rating_dict_41191_.npy',allow_pickle=True)#[uid,rat,iid]id从0开始
...
one=np.ones(shape=(41191,),dtype=int)
userid=userid+one
itemid=itemid+one
# print(max(userid))
# print(max(itemid))
# print(min(rating))
#rating=rating+one
# userid=train_rating[:,0]
# itemid=train_rating[:,2]
# rating=train_rating[:,1]
# one=np.ones(shape=(800167,),dtype=int)
# userid=userid+one
# itemid=itemid+one
#RW(userid,itemid,rating).get_adj_score(alpha=0.8,isuser=True)
RW(userid,itemid,rating,alpha=0.8).get_adj_score(isuser=True)
