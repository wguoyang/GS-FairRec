...
class DisClfGender(nn.Module):#判别器网络（性别）
...
class DisClfAge(nn.Module):#判别器网络（年龄）
    def __init__(self,embed_dim,out_dim,attribute,use_cross_entropy=True):
...
class DisClfOcc(nn.Module):#判别器网络（职业）
    def __init__(self,embed_dim,out_dim,attribute,use_cross_entropy=True):
        super(DisClfOcc, self).__init__()
        self.embed_dim = int(embed_dim) 
        self.attribute = attribute
        self.criterion = nn.CrossEntropyLoss().cuda()#torch.nn.BCELoss()#nn.CrossEntropyLoss()  ,这里用的是交叉熵损失函数
        self.out_dim = out_dim
        self.net = nn.Sequential(
            nn.Linear(self.embed_dim, int(self.embed_dim/2), bias=True),
            # nn.BatchNorm1d(num_features=self.embed_dim * 4),
...
class AttributeFilter(nn.Module):#过滤器网络
...
class LineGCN(nn.Module):
    def __init__(self, user_num, item_num, factor_num):
        ...
        self.embed_user = nn.Embedding(user_num, factor_num)#产生用户嵌入向量，维度是factor_num
        self.embed_item = nn.Embedding(item_num, factor_num) #产生物品嵌入向量，维度是factor_num
        self.factor_num=factor_num  

        nn.init.normal_(self.embed_user.weight, std=0.01)#从给定均值和标准差的正态分布N(mean, std)中生成值，填充输入的张量或变量
