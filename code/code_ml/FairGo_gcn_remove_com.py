...
# dataset_base_path='../../fairness0209/data/ml1m'#'#116,109 have. other use '..'
# saved_model_path='../../fairness0209'#116,109 have. other use '..'
...
shiyanmiaoshu='多敏感信息同时去除,LGCN+MLP+NF'
f = open('NF-FairRec_result1.txt', 'a')
...
class InforMax(nn.Module):
    ...
    def forward(self, adj_pos,user_batch,rating_batch,item_batch):      
        ...
        w_f=[1,1,1]#[1,2,1]#[5,2.5,0.5]#[1,2,1]#[5,2.5,0.5]#[1,1,1]#[5,2.5,0.5]#[1,1,1]#[5,2.5,0.5]#[1,1,1]#[5,2.5,0.5]#[1,1,1]#[5,2.5,0.5]#[3,1.5,0.5]#[5,2.5,0.5]#[1,1,1]#[5,2.5,0.5]
        ...
        user_person_f = (user_f1_tmp+user_f2_tmp+user_f3_tmp)/3.0
        item_person_f = (item_f1_tmp+item_f2_tmp+item_f3_tmp)/3.0
        #gcn_items_embedding0#item_f2_tmp#gcn_items_embedding0#item_f2_tmp
        ...
        d_loss_all=1.5*d_loss
        g_loss_all= 5*loss_p_square #- 1*d_loss_all#5
...
for epoch in range(100):
    ...
    user_e_f3 = f3_users_embedding.cpu().numpy()
    item_e_f1 = f1_i_emb.cpu().numpy()
    item_e_f2 = f2_i_emb.cpu().numpy()
    item_e_f3 = f3_i_emb.cpu().numpy()
    user_e_f=(user_e_f3+user_e_f2+user_e_f1)/3.0
    item_e_f=(item_e_f1+item_e_f2+item_e_f3)/3.0 

    user_e = user_e_f
    item_e = item_e_f#items_emb_gcn#item_e_f2#items_emb_gcn
    ...
    str_print_evl += str_f1f1_res
    ##occ
    f1p_one, f1r_one, f1res_p, f1res_r = pc_occ_train.clf_occ_all_pre(run_id, epoch, user_e, factor_num)
    # f1pone = np.mean(f1res_p)
    # print(f1pone)
    # f1rone = np.mean(f1res_r)
    # print(f1rone)
    ...
    str_print_evl += str_f1f1_res
    print(str_print_evl)
    f = open('NF-FairRec_result1.txt\n', 'a')
    f.write(str_print_evl)
    f.close()